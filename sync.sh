...
case "$2" in
	...
	remotedir=/home/rzhang/adjointpinn/
	;;

	"hpc")
	server=ziruz16@hpc3.rcic.uci.edu
	remotedir=/data/homezvol0/ziruz16/adjointpinn/
	;;

	"opengpu")
	server=ziruz16@lucy.ics.uci.edu
	remotedir=/home/ziruz16/adjointpinn/
	;;
...
basedir=/Users/Ray/project/adjointpinn/ # Update this to your base directory path
relpath=$(realpath --relative-to="$basedir" "$cdir")

# Append relative path to the remote directory
# if $relpath is not ., then append it to the remote directory
if [ "$relpath" != "." ]
then
...
filelist=(--exclude={'.*','logs','__pycache__'})
